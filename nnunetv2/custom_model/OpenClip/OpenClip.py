...
from nnunetv2.custom_model.encoder import OpenClipEncoder
...
class OpenClipUNETR(nn.Module):
    def __init__(self, embed_dim = 768, patch_size = 16, input_dim = 3, output_dim=3):
        ...
        self.encoder = OpenClipEncoder(freeze=True)
...
class OpenClipUNETR_ENC(nn.Module):
    def __init__(self, embed_dim = 768, patch_size = 16, input_dim = 3, output_dim=3):
        super(OpenClipUNETR_ENC, self).__init__()
        ...
        self.encoder = OpenClipEncoder(freeze=False)
        self.decoder1 = UNETRDecoder3D(embed_dim=self.embed_dim, patch_size=self.patch_size, input_dim=self.input_dim, output_dim=self.output_dim, ds = self.decoder.deep_supervision) 
