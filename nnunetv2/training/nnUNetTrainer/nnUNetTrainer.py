...
class nnUNetTrainer(object):
    ...
    def configure_optimizers(self):
        ...
        scale = 0.01
        target_iter = self.num_epochs * self.num_iterations_per_epoch
        lr_scheduler = CosineAnnealingWarmupRestarts(optimizer=optimizer, first_cycle_steps=target_iter, max_lr=self.initial_lr, min_lr=self.initial_lr*scale, warmup_steps=target_iter//20, gamma=0.8)
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        if self.grad_scaler is not None:
            ...
            self.lr_scheduler.step(self.current_global_steps)
            self.current_global_steps += 1
            #print(self.current_global_steps)
        else:
            l.backward()
            torch.nn.utils.clip_grad_norm_(self.network.parameters(), 12)
            self.optimizer.step()
    ...
    def on_epoch_end(self):
        ...
        if (current_epoch + 1) == 400:
            self.save_checkpoint(join(self.output_folder, 'checkpoint_400.pth'))
        ...
        if self.local_rank == 0:
            if not self.disable_checkpointing:
                ...
                checkpoint = {
                    ...
                    'current_global_steps': self.current_global_steps,
    ...
    def load_checkpoint(self, filename_or_checkpoint: Union[dict, str]) -> None:
        ...
        self.current_global_steps = checkpoint['current_global_steps']
